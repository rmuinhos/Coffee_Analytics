...
# Área Plantada (h)
df_area = pd.read_csv("CafeTotalSerieHistArabica_AreaPlantada.csv", sep=";")
...
# Produtividade (kg/h)
df_yield = pd.read_csv("CafeTotalSerieHistArabica_Produtividade.csv", sep=";") 
...
df_prod = pd.read_csv("CafeTotalSerieHistArabica_Producao.csv", sep=";")  
...
# Divide o dataframes de Produção em dois, de acordo com a bianualidade
df_prod_alta = df_prod[df_prod["Year"] % 2 == 0]  # Anos pares
df_prod_baixa = df_prod[df_prod["Year"] % 2 != 0]  # Anos ímpares

# Novas Área (h)
df_newarea = pd.read_csv("CafeTotalSerieHistArabica_AreaFormacao.csv", sep=";")  
...
for element, data in datasets.items():
    ...
    plt.xlabel("Ano")
    plt.ylabel("Valor")
    plt.title(f"Regressão para {element} - Fonte: Conab (Arábica) - Confiabilidade (R²): {r2_score:.4f}")
    plt.legend()
    plt.show()